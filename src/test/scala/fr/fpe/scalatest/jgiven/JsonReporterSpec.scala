...
class JsonReporterSpec extends FlatSpec {
  ...
  it should "serialize and deserialize InfoProvided" in {
    ...
    val infoProvided = InfoProvided(
      ...
      formatter = None,
      location =
        Some(TopOfMethod(className = "name.lemerdy.sebastian.scalatest.jgiven.TVSetSpec", methodId = "methodId")),
      payload = None,
      threadName = "pool-5-thread-2",
      timeStamp = Instant.parse("2019-06-25T21:55:00Z").toEpochMilli
    )
  ...
  it should "serialize and deserialize SuiteAborded" in {
    ...
    val suiteAborted = SuiteAborted(
      ...
      formatter = None,
      location = None,
      rerunner = Some("name.lemerdy.sebastian.scalatest.jgiven.TVSetSpec"),
      payload = None,
      threadName = "pool-5-thread-2",
      timeStamp = Instant.parse("2019-06-25T21:55:00Z").toEpochMilli
    )
